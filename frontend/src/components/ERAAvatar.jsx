...
const Hand = ({ side = "left" }) => (
  ...
  >
    ...
    {/* Thumb */}
    <ellipse cx={side === "left" ? 20 : 60} cy="66" rx="7" ry="10" fill={`url(#skin-${side})`} transform={`rotate(${side === "left" ? -25 : 25} ${side === "left" ? 20 : 60} 66)`} />
    {/* Finger creases */}
 ...
 * Hands rest idle, wave/gesture when `speaking`.
 * `listening` shows a dashed rotating ring behind the character.
 */
export const ERAAvatar = ({ size = 420, speaking = false, listening = false, className = "" }) => {
  return (
    <div
